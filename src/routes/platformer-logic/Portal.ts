...
/* PREVIOUS smooth style — kept for reference (radial gradients + rotating arc strokes):
	draw(ctx: CanvasRenderingContext2D) {
		const s = this.scale()
		if (s <= 0.001) return
		const { x, y } = this.pos
		const rx = this.radius * s
		const ry = this.radius * 1.35 * s
		const pulse = 0.5 + 0.5 * Math.sin(this.phase * 0.012)
		const warn = this.state === 'opening' ? 0.6 + 0.4 * pulse : 0.5 + 0.25 * pulse
		ctx.save()
		// Outer hostile halo (drawn in absolute space before the oval squash).
		const halo = ctx.createRadialGradient(x, y, 0, x, y, ry * 2.2)
		halo.addColorStop(0, `rgba(217, 70, 239, ${0.5 * warn})`) // fuchsia-500
		halo.addColorStop(0.5, `rgba(147, 51, 234, ${0.22 * warn})`) // purple-600
		halo.addColorStop(1, 'rgba(147, 51, 234, 0)')
		ctx.fillStyle = halo
		ctx.fillRect(x - ry * 2.2, y - ry * 2.2, ry * 4.4, ry * 4.4)
		// Rift body: draw a circle of radius rx, squashed vertically to the oval.
		ctx.translate(x, y)
		ctx.scale(1, ry / rx)
		const core = ctx.createRadialGradient(0, 0, 0, 0, 0, rx)
		core.addColorStop(0, 'rgba(10, 2, 20, 0.92)') // near-black centre
		core.addColorStop(0.7, `rgba(126, 34, 206, ${0.5 * warn})`) // purple-700
		core.addColorStop(1, `rgba(240, 171, 252, ${0.85 * warn})`) // fuchsia-300 rim
		ctx.fillStyle = core
		ctx.beginPath()
		ctx.arc(0, 0, rx, 0, Math.PI * 2)
		ctx.fill()
		// Rotating energy arcs for life.
		ctx.strokeStyle = `rgba(240, 171, 252, ${0.8 * warn})`
		ctx.lineWidth = 2
		for (let k = 0; k < 3; k++) {
			const a = this.phase * 0.02 + (k * Math.PI * 2) / 3
			ctx.beginPath()
			ctx.arc(0, 0, rx * (0.55 + 0.12 * k), a, a + 1.4)
			ctx.stroke()
		}
		ctx.restore()
	}
